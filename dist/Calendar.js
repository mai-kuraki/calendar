    ...
    _createClass(Calendar, [{
        ...
        value: function setMode(mode) {
            ...
            if (mode == 'week') {
                ...
                var curDay = this.curDate.getDay();
                var curFullYear = this.curDate.getFullYear();
                var curMonth = this.curDate.getMonth() + 1;
        ...
        key: 'getMonthDay',
        value: function getMonthDay(fullYear, month) {
            var monthDay = 30;
            switch (month) {
                case 1:
                    monthDay = 31;
        ...
        value: function initCalendar() {
            var _date = this.curDate;
            var fullYear = _date.getFullYear();
            var month = _date.getMonth() + 1;
            var today = new Date().getDate();
            var todayFullYear = new Date().getFullYear();
            var todayMonth = new Date().getMonth() + 1;
            var firstDay = this.getFirstDay(fullYear, month);
            var nextMonthFirstDay = this.getFirstDay(fullYear, month + 1);
            var monthDay = this.getMonthDay(fullYear, month);
            var lastFullYear = fullYear;
            var lastMonth = month - 1;
            if (lastMonth < 1) {
                lastMonth = 12;
                lastFullYear -= 1;
            }
            var lastMonthDay = this.getMonthDay(lastFullYear, lastMonth);
            var lastMonthData = [];
            ...
            if (nextMonthFirstDay > 0) {
                for (var _i = 0; _i < 7 - nextMonthFirstDay; _i++) {
                    nextMonthData.push(_i + 1);
                }
            }
            var monthData = [];
            for (var _i2 = 0; _i2 < monthDay; _i2++) {
                monthData.push(_i2 + 1);
            }

            if (this.mode == 'month') {
                var tpl = '';
                lastMonthData.map(function (data, k) {
                    tpl += '<div class="day-wrap">\n                <div class="day day-disable">' + data + '</div>\n            </div>';
                });
                monthData.map(function (data, k) {
                    tpl += '<div class="day-wrap">\n                    <div class="day ' + (data == today && fullYear == todayFullYear && month == todayMonth ? 'day-today' : '') + '">' + data + '</div>\n                </div>';
                });
                nextMonthData.map(function (data, k) {
                    tpl += '<div class="day-wrap">\n                <div class="day day-disable">' + data + '</div>\n            </div>';
                });
                (0, _jquery2.default)('.days').html(tpl);
                (0, _jquery2.default)('.curMonth').text(fullYear + '\u5E74' + this.formatNumber(month) + '\u6708');
            } else {
                var weekData = [];
                var startDay = this.curWeekDay - this.curWeek;
                var hasLastMonth = false;
                var hasNextMonth = false;
                for (var _i3 = 0; _i3 < 7; _i3++) {
                    var day = startDay + _i3;
                    if (day < 1) {
                        hasLastMonth = true;
                ...
                var _tpl = '';
                weekData.map(function (data, k) {
                    _tpl += '<div class="day-wrap">\n                    <div class="day ' + (data == today && fullYear == todayFullYear && month == todayMonth ? 'day-today' : '') + '">' + data + '</div>\n                </div>';
                });
                (0, _jquery2.default)('.days').html(_tpl);
                (0, _jquery2.default)('.curMonth').text(fullYear + '\u5E74' + (hasLastMonth ? this.formatNumber(month - 1) + '-' : '') + this.formatNumber(month) + (hasNextMonth ? '-' + this.formatNumber(month + 1) : '') + '\u6708');
            }
        }
        ...
        value: function preWeek() {
            ...
            if (this.curWeekDay < 1) {
                ...
                var lasMonthDay = this.getMonthDay(fullYear, month);
                this.preMonth();
...
(0, _jquery2.default)('.pre').click(function () {
    if (calendar.mode == 'month') {
        calendar.preMonth();
    } else {
        calendar.preWeek();
...
(0, _jquery2.default)('.today').click(function () {
    calendar.todayPosition();
});
(0, _jquery2.default)('.mode-week').click(function (e) {
    (0, _jquery2.default)('.mode-active').removeClass('mode-active');
    (0, _jquery2.default)(e.target).addClass('mode-active');
    calendar.setMode('week');
});
(0, _jquery2.default)('.mode-month').click(function (e) {
    (0, _jquery2.default)('.mode-active').removeClass('mode-active');
    (0, _jquery2.default)(e.target).addClass('mode-active');
